...
export async function getUserData(userId) {
  try {
    const {
      data: { user },
    } = await axios.get(`${import.meta.env.VITE_API_ENDPOINT}/user/${userId}`);
    return user;
  } catch (error) {
    console.log(error);
  }
...
export async function saveUserAddress(address, city, pincode, state, userId) {
  try {
    const {
      data: { user },
    } = await axios.post(`${import.meta.env.VITE_API_ENDPOINT}/user/address`, {
      address,
      city,
      pincode,
      state,
...
export async function getProductsData() {
  try {
    const {
      data: { productsData },
    } = await axios.get(`${import.meta.env.VITE_API_ENDPOINT}/products`);
    return productsData;
  } catch (error) {
    console.log({ error });
  }
...
export async function getCartData(userId) {
  try {
    const {
      data: { cart },
    } = await axios.get(`${import.meta.env.VITE_API_ENDPOINT}/cart/${userId}`);
    return cart;
  } catch (error) {
    console.log({ error });
  }
...
export async function getWishListData(userId) {
  try {
    const {
      ...
      `${import.meta.env.VITE_API_ENDPOINT}/wishlist/${userId}`
    );
    return wishList;
  } catch (error) {
    console.log({ error });
...
export async function updateCartData(userId, book) {
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_ENDPOINT}/cart/${userId}`,
